...
class M_AutoUpdate extends C_Base_Module
{
    ...
    function _get_api_url()
    {
        if (is_null($this->_api_url)) {
            $this->_api_url = 'https://members.photocrati.com/api/';
        }
    ...
    function get_license($product = null)
    {
        ...
        $license_default = defined('PHOTOCRATI_LICENSE_DEFAULT') ? PHOTOCRATI_LICENSE_DEFAULT : get_option('photocrati_license_default');
...
#				}
    ...
    function cleanup_command($install_path, $activate_path, $other_path, $command_info)
    {
        ...
        /**
         * NGG Plus 1.6.23 included nggallery-pro.php by mistake, and needs to be removed
         */
        if (defined('NGG_PLUS_PLUGIN_VERSION')) {
            $to_remove = path_join(
                C_Component_Registry::get_instance()->get_module_dir('photocrati-nextgen-plus'),
                'nggallery-pro.php'
            );

            // First try removing the WP-way
            $success = FALSE;
            if ($wp_filesystem) {
                if ($wp_filesystem->exists($to_remove)) {
                    $success = $wp_filesystem->delete($to_remove);
                }
            }

            // If that fails, then attempt using unlink
            if (!$success && @file_exists($to_remove)) {
                $success = @unlink('nggallery-pro.php');
                if (!$success) {
                    file_put_contents($to_remove, "");
                }
            }
        }
    }
